...
static void
dbg_print_SDL_key(SDL_Keysym *sym, SDL_bool pressed, SDL_bool repeat)
{
    char message[512];
    char *spot;
    size_t left;
...
loop()
{
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            case SDL_KEYDOWN:
            case SDL_KEYUP:
                dbg_print_SDL_key(&event.key.keysym,
                        (event.key.state == SDL_PRESSED) ? SDL_TRUE : SDL_FALSE,
                        (event.key.repeat) ? SDL_TRUE : SDL_FALSE);
                break;
            case SDL_QUIT:
                done = 1;
                break;
